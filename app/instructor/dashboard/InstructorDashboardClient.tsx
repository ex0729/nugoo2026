...
type ClassRequest = {
  ...
  assignment: { id: string; role: "lead" | "assistant"; fee_snapshot: number; assigned_at: string; acknowledged_at: string | null } | null;
};
...
export default function InstructorDashboardClient({ instructorName }: { instructorName: string }) {
  ...
  const assignedRequests = requests.filter(request => request.class?.status === "assigned" && request.assignment);
  ...
  async function acknowledge(assignmentId: string) {
    setSavingId(assignmentId); setError("");
    const response = await fetch("/api/instructor/requests", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ assignmentId }) });
    setSavingId("");
    if (!response.ok) { setError("배정 확인을 저장하지 못했습니다."); return; }
    await loadDashboard();
  }
  ...
  return <main className="instructor-dashboard-page">
    ...
    <div className="instructor-dashboard-shell"><nav className="instructor-dashboard-nav"><a className="active" href="#overview"><span>⌂</span>홈</a><a href="#notifications"><span>◷</span>알림 {unreadCount ? `(${unreadCount})` : ""}</a><a href="#requests"><span>↗</span>수업 요청</a><a href="#schedule"><span>□</span>내 일정</a></nav><section className="instructor-dashboard-content" id="overview"><div className="instructor-dashboard-welcome"><div><p className="section-kicker">INSTRUCTOR DASHBOARD</p><h1>{instructorName} 강사님, 반갑습니다</h1><p>요청받은 역할과 수업료를 확인하고 마감 전에 응답해 주세요.</p></div><span>✓ 승인된 계정</span></div>
      ...
      <section className="instructor-request-list" id="schedule"><header><div><p className="section-kicker">CONFIRMED SCHEDULE</p><h2>내 확정 일정</h2></div><span>{assignedRequests.length}건</span></header>{assignedRequests.map(request => request.class && request.assignment && <article className="instructor-request-card confirmed-schedule" key={request.assignment.id}><div><StatusChip status="assigned" /><h3>{request.class.institution} · {request.class.title}</h3><p>{request.class.class_date} · {request.class.start_time.slice(0,5)}~{request.class.end_time.slice(0,5)} · {request.class.address}</p><small>{request.assignment.role === "lead" ? "주강사" : "보조강사"} · {won(request.assignment.fee_snapshot)}</small></div>{request.assignment.acknowledged_at ? <span className="assignment-acknowledged">✓ 배정 확인 완료 · {new Date(request.assignment.acknowledged_at).toLocaleString("ko-KR")}</span> : <button className="button primary" disabled={savingId === request.assignment.id} onClick={() => void acknowledge(request.assignment!.id)}>{savingId === request.assignment.id ? "저장 중…" : "배정 확인"}</button>}</article>)}{!loading && assignedRequests.length === 0 && <div className="instructor-dashboard-empty"><p>아직 확정된 일정이 없습니다.</p></div>}</section>
      <section className="instructor-request-list" id="requests"><header><div><p className="section-kicker">CLASS REQUESTS</p><h2>내 수업 요청</h2></div><span>{requests.length}건</span></header>{loading && <div className="instructor-dashboard-empty"><p>수업 요청을 불러오는 중입니다.</p></div>}{!loading && requests.map(request => request.class && <article className="instructor-request-card" id={`request-${request.class.id}`} key={request.id}><div><StatusChip status={request.class.status} /><h3>{request.class.institution} · {request.class.title}</h3><p>{request.class.class_date} · {request.class.start_time.slice(0,5)}~{request.class.end_time.slice(0,5)} · {request.class.address}</p><small>{request.class.status === "assigned" ? request.assignment ? `배정됨 · ${request.assignment.role === "lead" ? "주강사" : "보조강사"}` : "배정 완료 · 미선정" : `응답 마감 ${new Date(request.class.response_deadline).toLocaleString("ko-KR")}`}</small></div><div className="instructor-role-responses">{request.responses.map(item => { const responseOpen = ["registered", "recruiting", "reviewing", "assignment_needed"].includes(request.class!.status) && new Date(request.class!.response_deadline) > new Date(); return <section key={item.id}><header><b>{item.role === "lead" ? "주강사" : "보조강사"}</b><strong>{item.role === "lead" ? won(request.class!.lead_fee) : `1인당 ${won(request.class!.assistant_fee)}`}</strong><span>{item.status === "pending" ? "미응답" : item.status === "available" ? "가능" : item.status === "conditional" ? "조건부 가능" : "불가능"}</span></header><p>{request.class!.fee_notes || "추가 수업료 안내 없음"}</p><input aria-label="조건 입력" disabled={!responseOpen} placeholder="조건부 가능일 때 조건을 입력하세요" value={conditions[item.id] ?? item.condition ?? ""} onChange={event => setConditions(current => ({ ...current, [item.id]: event.target.value }))} />{responseOpen ? <div><button disabled={savingId === item.id} onClick={() => respond(item.id, "available")}>가능</button><button disabled={savingId === item.id} onClick={() => respond(item.id, "conditional")}>조건부 가능</button><button disabled={savingId === item.id} onClick={() => respond(item.id, "unavailable")}>불가능</button></div> : <p className="response-closed">응답 기간이 종료되었습니다.</p>}</section>; })}</div></article>)}{!loading && requests.length === 0 && <div className="instructor-dashboard-empty"><span>↗</span><h3>현재 응답할 수업 요청이 없습니다</h3><p>관리자가 모집 요청을 보내면 역할과 수업료가 이곳에 표시됩니다.</p></div>}</section>
    </section></div>
  </main>;
}

function StatusChip({ status }: { status: string }) {
  return <span className="instructor-request-status">{status === "assigned" ? "배정 완료" : status === "completed" ? "수업 완료" : status === "cancelled" ? "취소" : "모집 중"}</span>;
}
