...
export async function GET() {
  ...
  const [classResult, responseResult] = await Promise.all([
  ...
  if (classResult.error || responseResult.error) return NextResponse.json({ error: "requests_unavailable" }, { status: 500 });
  const classes = new Map((classResult.data ?? []).map(item => [item.id, item]));
  return NextResponse.json({ requests: (targets ?? []).map(target => ({ ...target, class: classes.get(target.class_id) ?? null, responses: (responseResult.data ?? []).filter(response => response.target_id === target.id) })) });
}
...
export async function PATCH(request: Request) {
  ...
  const supabase = await createClient();
  const { data, error } = await supabase.from("class_recruitment_responses").update({
    status: body.status,
    condition: body.status === "conditional" ? body.condition?.trim() : null,
    responded_at: new Date().toISOString(),
    updated_at: new Date().toISOString(),
  }).eq("id", body.responseId).select("id,status,condition,responded_at").single();
