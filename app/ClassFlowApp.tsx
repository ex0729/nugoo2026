...
const classTargetOptions = ["중학생", "초등학생", "고등학생", "성인", "유아"] as const;
const classGradeOptions = ["1학년", "2학년", "3학년", "4학년", "5학년", "6학년", "기타"] as const;

function normalizedGradeSelection(value: string) {
  if (value === "전 학년") return classGradeOptions.slice(0, 6);
  if (value === "1~2학년" || value === "1–2학년") return ["1학년", "2학년"];
  const selected = value.split(",").map(item => item.trim()).filter((item): item is typeof classGradeOptions[number] => classGradeOptions.includes(item as typeof classGradeOptions[number]));
  if (selected.length) return [...new Set(selected)];
  return value && value !== "미정" ? ["기타"] : [];
}

function GradeSelector({ value, onChange }: { value: string[]; onChange: (grades: string[]) => void }) {
  const toggle = (grade: string) => onChange(value.includes(grade) ? value.filter(item => item !== grade) : [...value, grade]);
  return <fieldset className="grade-selector"><legend>학년 <small>복수 선택 가능</small></legend><div className="grade-option-grid">{classGradeOptions.map(grade => <label className={value.includes(grade) ? "selected" : ""} key={grade}><input type="checkbox" checked={value.includes(grade)} onChange={() => toggle(grade)} /><span>{grade}</span></label>)}</div></fieldset>;
}
...
function ClassForm({ close, onCreated }: { close: () => void; onCreated: (item: StoredClass) => void }) {
  ...
  const draft = useRef({
    title: "", institution: "", contact: "", classDate: initialDates.current.classDate,
    startTime: "10:00", endTime: "12:00", address: "", targetGroup: "",
    grade: "", participantCount: 0, description: "",
    leadFee: 0, assistantFee: 0, feeNotes: "",
  ...
  const goNext = () => {
    ...
    if (!value.title.trim() || !value.institution.trim() || !value.address.trim() || !value.targetGroup || grades.length === 0 || value.participantCount < 1) {
      setError("수업명, 기관명, 장소, 대상, 학년, 참여 인원을 입력해 주세요.");
      return;
    }
    setError("");
    setStep(2);
...
function ClassEditForm({ classItem, close, onSaved }: { classItem: StoredClass; close: () => void; onSaved: (item: StoredClass) => void }) {
  ...
  const initialGrades = normalizedGradeSelection(classItem.grade);
  ...
  const [form, setForm] = useState({
    ...
    targetGroup: classItem.target_group, grade: initialGrades.join(", ") || classItem.grade, participantCount: classItem.participant_count,
