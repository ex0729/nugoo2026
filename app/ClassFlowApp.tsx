...
function ClassesScreen({ onCreate, goRequests }: { onCreate: () => void; goRequests: () => void }) {
  ...
  const visible = filter === "전체" ? classes : classes.filter(c => c.status === filter);
  return <div className="screen-stack"><section className="toolbar"><div className="search"><span aria-hidden="true">⌕</span><input aria-label="수업 검색" placeholder="기관명 또는 수업명 검색" /></div><div className="filter-tabs">{["전체", "응답 대기", "배정 필요", "배정 완료"].map(f => <button className={filter === f ? "active" : ""} onClick={() => setFilter(f)} key={f}>{f}</button>)}</div><button className="button primary" onClick={onCreate}>＋ 새 수업</button></section>
    <section className="panel class-table-panel"><div className="list-summary"><p>전체 수업 <b>{visible.length}</b></p><div><button className="view-toggle active">목록</button><button className="view-toggle">주간 일정</button></div></div><div className="class-table"><div className="class-table-head"><span>수업 정보</span><span>일정·장소</span><span>모집 현황</span><span>응답 마감</span><span>상태</span><span /></div>{visible.map(item => <div className="class-row" key={item.id}><span><b>{item.title}</b><small>{item.institution}</small></span><span><b>{item.date} · {item.time}</b><small>{item.place}</small></span><span><small>주강사 {item.lead}</small><small>보조강사 {item.assistant}</small></span><span className={item.deadline.includes("지남") ? "text-red" : ""}>{item.deadline}</span><span><StatusBadge tone={item.tone}>{item.status}</StatusBadge></span><span><button className="row-action" onClick={goRequests}>{item.status === "응답 대기" || item.status === "배정 필요" ? "현황 보기" : "상세"}</button></span></div>)}</div></section>
  </div>;
}
...
function RequestsScreen() {
  ...
  const filtered = statusFilter === "전체" ? candidates : candidates.filter(c => c.status === statusFilter);
  const toggleSelected = (name: string) => setSelected(v => v.includes(name) ? v.filter(n => n !== name) : [...v, name]);
  const toggleAssign = (name: string) => setAssigned(v => v.includes(name) ? v.filter(n => n !== name) : [...v, name]);
  return <div className="request-layout">
    ...
    <section className="panel response-board"><div className="board-head"><div className="filter-tabs">{["전체", "가능", "조건부", "불가능", "미응답"].map(f => <button className={statusFilter === f ? "active" : ""} onClick={() => setStatusFilter(f)} key={f}>{f}{f === "미응답" ? " 4" : ""}</button>)}</div><div className="board-actions">{selected.length > 0 && <button className="button secondary">선택 {selected.length}명 재알림</button>}<div className="search compact"><span>⌕</span><input aria-label="강사 검색" placeholder="강사 검색" /></div></div></div>
      <div className="candidate-table"><div className="candidate-head"><span><input type="checkbox" aria-label="전체 선택" /></span><span>강사</span><span>모집 역할</span><span>응답 상태</span><span>조건·충돌</span><span>배정 후보</span></div>{filtered.map(c => <div className="candidate-row" key={c.name}><span><input type="checkbox" aria-label={`${c.name} 선택`} checked={selected.includes(c.name)} onChange={() => toggleSelected(c.name)} /></span><span className="person"><b>{c.initials}</b><span>{c.name}<small>{c.subject} · {c.region}</small></span></span><span><RoleBadge role={c.role} /></span><span><StatusBadge tone={c.status === "가능" ? "green" : c.status === "조건부" ? "amber" : c.status === "불가능" ? "red" : "gray"}>{c.status}</StatusBadge><small className="cell-note">{c.time}</small></span><span>{c.conflict ? <span className="conflict">! 확정 일정과 30분 겹침</span> : c.condition ? <span className="condition-text">“{c.condition}”</span> : <span className="muted">특이사항 없음</span>}</span><span>{c.status === "가능" || c.status === "조건부" ? <button className={`assign-toggle ${assigned.includes(c.name) ? "selected" : ""}`} onClick={() => toggleAssign(c.name)}>{assigned.includes(c.name) ? "✓ 선택됨" : "후보 선택"}</button> : <span className="muted">—</span>}</span></div>)}</div>
    </section>
    <section className="assignment-bar"><div><span>현재 배정 후보</span><div className="selected-people">{assigned.map((name, i) => <b key={name}>{name}<small>{i === 0 ? "주강사" : "보조강사"}</small></b>)}</div></div><p><span>주강사 <b>1/1</b></span><span>보조강사 <b>{Math.max(0, assigned.length - 1)}/2</b></span></p><button className="button primary" disabled={assigned.length < 3}>최종 배정 확인</button></section>
  </div>;
}
...
function InstructorsScreen() {
  return <div className="screen-stack"><section className="toolbar"><div className="search"><span>⌕</span><input aria-label="강사 검색" placeholder="강사명, 과목, 지역 검색" /></div><div className="filter-tabs"><button className="active">전체 15</button><button>활성 13</button><button>승인 대기 2</button></div><button className="button secondary">강사 초대</button></section><section className="instructor-grid">{instructors.map((p, i) => <article className="panel instructor-card" key={p.name}><div className="instructor-card-head"><span className={`avatar avatar-${i % 4}`}>{p.name[0]}</span><div><h3>{p.name}</h3><p>{p.subjects}</p></div><StatusBadge tone={p.state === "활성" ? "green" : "amber"}>{p.state}</StatusBadge></div><dl><div><dt>활동 지역</dt><dd>{p.region}</dd></div><div><dt>확정 수업</dt><dd>{p.classes}회</dd></div><div><dt>응답률</dt><dd>{p.rate}</dd></div></dl><div className="instructor-card-actions"><button>프로필 보기</button><button>일정 확인</button></div></article>)}</section></div>;
}
